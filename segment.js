...
// Blue numbered waypoint
function addNumberedMarker(map, lat, lng, number) {
  const svgMarkup = `
      <svg xmlns="http://www.w3.org/2000/svg" width="28" height="28">
        <circle cx="14" cy="14" r="12" fill="#1E90FF" stroke="white" stroke-width="2"/>
        <text x="14" y="18" font-size="12" font-weight="bold" text-anchor="middle" fill="white">${number}</text>
      </svg>`;
...
function handleRoute(result, type) {
  if (result.routes.length) {
    ...
    if (type === "forward") {
      addNumberedMarker(
        map,
        coords[Math.floor(coords.length * 0.25)].lat,
        coords[Math.floor(coords.length * 0.25)].lng,
        1
      );
      addNumberedMarker(
        map,
        coords[Math.floor(coords.length * 0.5)].lat,
        coords[Math.floor(coords.length * 0.5)].lng,
        2
      );
      addNumberedMarker(
        map,
        coords[Math.floor(coords.length * 0.75)].lat,
        coords[Math.floor(coords.length * 0.75)].lng,
        3
      );
    }
...
function moveMarker() {
  ...
  if (index >= coords.length) {
    if (currentRoute === "forward") {
      ...
      index = 0;
      movingMarker.setCenter(backwardCoords[0]); // reset immediately
    } else {
...
document.querySelector("#speedUpBtn").addEventListener("click", function () {
...
document.querySelector("#speedDownBtn").addEventListener("click", function () {
  speed = speed * 2;
  if (intervalId) {
    clearInterval(intervalId);
    intervalId = setInterval(moveMarker, speed);
...
iconButton.addEventListener("click", function () {
  window.location.href = "/track-history.html";
});
